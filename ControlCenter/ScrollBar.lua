...
do
    ...
    function ArrowButtonMixin:OnMouseUp()
        ...
        if self:GetParent().ScrollView.StopSteadyScroll then
...
do
    ...
    function ThumbButtonMixin:OnMouseDown(button)
        if button == "LeftButton" then
            ...
            self:LockHighlight()
            ADT.LandingPageUtil.PlayUISound("ScrollBarThumbDown")
        end
    end
...
do
    function ScrollBarMixin:OnLoad()
        ...
        API.Mixin(self.UpArrow, ArrowButtonMixin)
        API.Mixin(self.DownArrow, ArrowButtonMixin)
        API.Mixin(self.Thumb, ThumbButtonMixin)
        self.UpArrow:OnLoad()
    ...
    function ScrollBarMixin:UpdateVisibleExtentPercentage()
        if not self.ScrollView or not self.ScrollView.GetScrollRange then return end
   ...
   function ScrollBarMixin:SetValueByRatio(ratio)
        ...
        self.ratio = ratio
        self.Thumb:SetPoint("TOP", self.Rail, "TOP", 0, -ratio * (self.thumbRange or 0))
    end
    ...
    function ScrollBarMixin:SetScrollable(scrollable)
        if scrollable then
            ...
            self.UpArrow:Show()
            self.DownArrow:Show()
            self.isTop = self.ScrollView and self.ScrollView.IsAtTop and self.ScrollView:IsAtTop() or true
            self.isBottom = self.ScrollView and self.ScrollView.IsAtBottom and self.ScrollView:IsAtBottom() or false
            self:SetAlpha(1)
        else
            self.Thumb:Hide()
            self.UpArrow:Hide()
    ...
    function ScrollBarMixin:OnUpdate_ThumbDragged(elapsed)
        ...
        if self.ScrollView and self.ScrollView.SnapToRatio then
    ...
    function ScrollBarMixin:OnUpdate_ArrowPushed(elapsed)
        ...
        if self.t > 0.3 then
            self.t = 0
            if self.ScrollView and self.ScrollView.SteadyScroll then
    ...
    function ScrollBarMixin:ScrollToMouseDownPosition()
        ...
        if self.ScrollView and self.ScrollView.ScrollToRatio then
...
do
    ...
    function DebugScrollView:IsAtTop() return true end
    function DebugScrollView:IsAtBottom() return false end
    function DebugScrollView:SteadyScroll() end
    function DebugScrollView:StopSteadyScroll() end
    function DebugScrollView:GetScrollRange() return 0 end
    function DebugScrollView:GetHeight() return 100 end
end


local function CreateScrollBarWithDynamicSize(parent)
    local f = CreateFrame("Frame", nil, parent, "ADTScrollBarWithDynamicSizeTemplate")

    API.Mixin(f, ScrollBarMixin)
    f:OnLoad()
    f:SetTexure("Interface/AddOns/AdvancedDecorationTools/Art/ControlCenter/SettingsPanelWidget.png")
